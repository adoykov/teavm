...
class TDerInputBuffer extends ByteArrayInputStream implements Cloneable {
    ...
    TDerInputBuffer(byte[] buf) {
        this(buf, true);
    }

    TDerInputBuffer(byte[] buf, boolean allowBER) {
        super(buf);
        this.allowBER = allowBER;
    }

    TDerInputBuffer(byte[] buf, int offset, int len, boolean allowBER) {
    ...
    TDerInputBuffer dup() {
        try {
            TDerInputBuffer retval = (TDerInputBuffer)clone();
    ...
    public boolean equals(Object other) {
        if (other instanceof TDerInputBuffer) {
            return equals((TDerInputBuffer)other);
        } else {
            return false;
        }
    }

    boolean equals(TDerInputBuffer other) {
        if (this == other) {
            return true;
        }
    ...
    TBitArray getUnalignedBitString() throws IOException {
        ...
        TBitArray bitArray = new TBitArray(length, bits);
        pos = count;
        return bitArray;
    }
